...
class ExtensionBundle:
    ...
    **Extension Bundle Specification, version 4**
    ...
    def save(
        ...
        Enable `pycOnly` if only compiled python files should be embedded into the extension.
        `pycOnly` can also be a list of python version. Providing as `pycExclude` list will
        exclude given scripts for being compiled.
        """
        ...
        if pycOnly:
            # remove all unwanted .py files
            # and create .pyc files
            # exclude the main, uninstall script and menu .py files,
            # they are executed by a script
            # if pyc is just true: build pyc for the current RF python
            # if pyc is is a list of ["3.8", "3.7"] or ["current", "3.6"]
            if isinstance(pycOnly, bool):
                pycOnly = ["current"]
            ...
            for pyVersion in reversed(pycOnly):
                if pyVersion == "current":
                    pycVersion = f"{sys.version_info.major}.{sys.version_info.minor}"
                else:
                    pycVersion = pyVersion
                if len(pycOnly) > 1 and pyVersion != pycOnly[0]:
                    self._libName = f"{libName}.{pycVersion}"
                else:
                ...
                if self.uninstallScript:
                    uninstallFile = self.uninstallScriptPath()
                    excludedPyFiles.append(uninstallFile)
...
for pyFile in {pyFiles}:
    print("begin compiling %s" % pyFile)
    # create a .pyc file
...
"""
                with tempfile.TemporaryDirectory() as codeRoot:
                    codeFile = os.path.join(codeRoot, "buildPYC.py")
    ...
    @classmethod
    def _requirementCompareVersionCompareEqualOrBigger(self, v1, v2):
        return version.Version(v1) >= version.Version(v2)
